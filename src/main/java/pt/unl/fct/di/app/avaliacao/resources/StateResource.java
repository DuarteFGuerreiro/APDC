...
import pt.unl.fct.di.app.avaliacao.util.StateData;
...
public class StateResource {
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public Response doState(StateData data) {
		LOG.fine("Attempt to change state of user: " + data.getUsername() + " by admin " + data.getAdmin());
		...
		try {
			...
			if (user == null) {
				LOG.warning("Failed to change state for username: " + data.getUsername());
				txn.rollback();
				return Response.status(Status.FORBIDDEN).build();
			}
			String adminRole = admin.getString("user_role");
			String userRole = user.getString("user_role");
			if (adminRole.equals("USER") || (adminRole.equals("GBO") && !userRole.equals("USER")) || userRole.equals("SU")) {
				LOG.warning("Failed state change for username: " + data.getUsername() + " admin does not have permission.");
				txn.rollback();
				return Response.status(Status.FORBIDDEN).build();	
			}
			if (adminRole.equals("GA") &&  userRole.equals("GA")) {
				LOG.warning("Failed role change for username: " + data.getUsername() + " admin is outranked.");
				txn.rollback();
				return Response.status(Status.FORBIDDEN).build();		
			}
			if (token == null) {
				LOG.warning("Failed token change state for username: " + data.getUsername());
				txn.rollback();
				return Response.status(Status.FORBIDDEN).build();
			}
			if (!token.getString("token_username").equals(data.getAdmin())) {
				LOG.warning("Failed change state for username: " + data.getUsername() + " wrong token for admin.");
				txn.rollback();
				return Response.status(Status.FORBIDDEN).build();
			}
			if (token.getLong("token_expirationData") < System.currentTimeMillis()) {
				LOG.warning("Failed change for username: " + data.getUsername() + " admin token expired.");
				txn.rollback();
				return Response.status(Status.FORBIDDEN).build();
			}
			...
			user = Entity.newBuilder(userKey).set("user_username", user.getString("user_username"))
					...
					.build();
			txn.put(user);
			LOG.info("User state changed " + data.getUsername());
			txn.commit();
